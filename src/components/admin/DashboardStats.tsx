...
const StatCard = ({ label, value, color }: StatCardProps) => (
  <div style={{
    ...
    <p style={{ fontSize: '2rem', fontWeight: 600, color, margin: 0 }}>{value}</p>
    <p style={{ fontSize: '0.82rem', color: '#6b6b6b', marginTop: '0.25rem' }}>{label}</p>
  </div>
);
...
const DashboardStats = ({ bookings }: Props) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      ...
      {/* Today */}
      <div>
        {sectionLabel(`Today — ${new Date().toLocaleDateString('en-AU', {
          weekday: 'long', day: 'numeric', month: 'long'
        })}`)}
        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
          <StatCard label="Total" value={todayStats.total} color="#c9a96e" />
          <StatCard label="Confirmed" value={todayStats.confirmed} color="#1d9e75" />
          <StatCard label="Pending" value={todayStats.pending} color="#ef9f27" />
          <StatCard label="Cancelled" value={todayStats.cancelled} color="#e24b4a" />
        </div>
      </div>

      {/* This week */}
      <div>
        {sectionLabel(`This week — ${new Date(start).toLocaleDateString('en-AU', {
        ...
        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
          ...
          <StatCard label="Confirmed" value={weekStats.confirmed} color="#1d9e75" />
          <StatCard label="Pending" value={weekStats.pending} color="#ef9f27" />
          <StatCard label="Cancelled" value={weekStats.cancelled} color="#e24b4a" />
        </div>
      </div>
